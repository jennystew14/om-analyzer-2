...
def generate_pdf_memo(d, models, overrides):
    ...
    pdf.cell(0, 9, prop.get("name", "Property"), align="L")
    pdf.ln(7)
    pdf.set_font("Helvetica", "", 10)
    pdf.set_text_color(168, 196, 224)
    pdf.cell(0, 5, f"{prop.get('address','')}  |  {prop.get('city_state','')}  |  {prop.get('units','?')} Units  |  Built {prop.get('year_built','?')}", align="L")
    ...
    color_map = {"PROCEED": (212, 237, 218, 40, 167, 69),
                 "WATCH":   (255, 243, 205, 255, 193, 7),
                 "PASS":    (248, 215, 218, 220, 53, 69)}
    bg = color_map.get(rec, (240, 240, 240, 100, 100, 100))
    pdf.set_fill_color(bg[0], bg[1], bg[2])
    pdf.rect(16, 44, 178, 22, "F")
    pdf.set_y(46)
    pdf.set_font("Helvetica", "B", 13)
    pdf.set_text_color(bg[3], bg[4], bg[5])
    pdf.cell(0, 7, f"VERDICT: {rec}  —  {verdict.get('one_liner','')}", align="C")
    pdf.ln(6)
    pdf.set_font("Helvetica", "", 8)
    pdf.set_text_color(80, 80, 80)
    pdf.cell(0, 5, f"Confidence: {verdict.get('confidence','')}  |  Asking: {fmt_d(prop.get('asking_price'))}  |  {fmt_d(prop.get('price_per_unit'))}/unit", align="C")
    pdf.set_text_color(0, 0, 0)
    pdf.set_y(70)
    ...
    def bullet(text, flag="•", color=(60, 60, 60)):
        ...
        pdf.multi_cell(170, 4.5, f"{flag}  {text}")
        pdf.set_text_color(0, 0, 0)
    ...
    left_items = [
        ...
        ("Broker Pro Forma Cap", fmt_pct(income.get("broker_pro_forma_cap_rate"))),
    ]
    right_items = [
    ...
    if income.get("t12_noi") and income.get("broker_projected_noi"):
        ...
        pdf.cell(0, 5, f"  Broker NOI uplift vs T-12: {uplift:+.1f}% — verify assumptions", ln=True)
        pdf.set_text_color(0, 0, 0)
    pdf.ln(2)
    ...
    if exp.get("expense_notes"):
        ...
        pdf.multi_cell(178, 4.5, f"Notes: {exp.get('expense_notes','')}")
        pdf.set_text_color(0,0,0)
    pdf.ln(2)
    ...
    if rc.get("notes"):
        pdf.set_x(16); pdf.set_font("Helvetica", "I", 7.5); pdf.set_text_color(100,100,100)
        pdf.multi_cell(178, 4.5, rc.get("notes",""))
        pdf.set_text_color(0,0,0)
    pdf.ln(2)
    ...
    pdf.set_font("Helvetica", "B", 8)
    pdf.cell(0, 5, f"Recommendation: {verdict.get('recommendation','')}  (Confidence: {verdict.get('confidence','')})", ln=True)
    pdf.ln(1)
...
# ──────────────────────────────────────────────────
    ...
    with tab4:
        ...
        with c2:
            ...
            st.metric("In-Place Cap Rate", fmt_pct(income.get("in_place_cap_rate")))

...
def clean(text):
    """Strip characters outside latin-1 range so fpdf/Helvetica doesn't choke."""
    if not text: return ""
    replacements = {
        "\u2014": "-", "\u2013": "-", "\u2018": "'", "\u2019": "'",
        "\u201c": '"', "\u201d": '"', "\u2022": "*", "\u2026": "...",
...
def generate_pdf_memo(d, models, overrides):
    ...
    pdf.cell(0, 7, clean(f"VERDICT: {rec}  -  {verdict.get('one_liner','')}"), align="C")
    pdf.ln(6)
    pdf.set_font("Helvetica", "", 8)
    pdf.set_text_color(80, 80, 80)
    pdf.cell(0, 5, clean(f"Confidence: {verdict.get('confidence','')}  |  Asking: {fmt_d(prop.get('asking_price'))}  |  {fmt_d(prop.get('price_per_unit'))}/unit"), align="C")
    pdf.set_text_color(0, 0, 0)
    pdf.set_y(70)
    ...
    def bullet(text, flag="*", color=(60, 60, 60)):
        ...
        pdf.multi_cell(170, 4.5, clean(f"{flag}  {text}"))
        pdf.set_text_color(0, 0, 0)
